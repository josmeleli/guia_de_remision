...
<div id="hamburger-icon">
    ...
    <div id="sidebar" class="sidebar overflow-auto">
        ...
        <ul class="list-unstyled components">
            ...
            <li>
                <a href="#" class="sidebar-link ajustes-link">
                    <div class="div-icon">
                        <i class="fas fa-cogs sidebar-icon"></i>
                    </div>
                    <div class="text-sidebar">
                        <span class="d-none d-md-inline">Ajustes</span>
                    </div>
                </a>
            </li>
        </ul>
...
#hamburger-icon {
    position: absolute;
    top: 3%;
    margin-left: 16%; /* Ajuste para que esté al lado del sidebar */
    cursor: pointer;
}

.sidebar.collapsed #hamburger-icon {
    margin-left: 250px; /* Ajuste para que esté al lado del sidebar colapsado */
}

    .submenu.active {
        max-height: 200px; /* Ajusta el valor según la altura máxima del submenú */
        opacity: 1;
    }
    .rotate {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const textElements = document.querySelectorAll('.sidebar-link .d-none');
        const submenuItems = document.querySelectorAll('.submenu');
    
        // Agrega un evento de clic al icono de hamburguesa
        hamburgerIcon.addEventListener('click', function() {
            sidebar.classList.toggle('collapsed'); // Alternar la clase 'collapsed' para mostrar u ocultar el sidebar
    
            // Alternar la visibilidad de los textos de los iconos
            textElements.forEach(function(element) {
                element.classList.toggle('hide-text');
            });
    
            // Ocultar los submenús si están abiertos
            submenuItems.forEach(function(item) {
                item.classList.remove('show');
            });
        });
    
        // Agrega un evento de clic a los elementos del menú con submenú
        submenuItems.forEach(function(item) {
            item.previousElementSibling.addEventListener('click', function(event) {
                event.preventDefault();
                item.classList.toggle('show'); // Alternar la clase 'show' para mostrar u ocultar el submenú
            });
        });
    });
    </script>
